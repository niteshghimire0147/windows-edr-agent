...
from modules.alert_engine      import (
    ...
    CATEGORY_SERVICE, CATEGORY_UNAUTHORIZED,
)
...
def build_dashboard(
    ...
    for sev, style in [(SEVERITY_HIGH, "bold red"), (SEVERITY_MEDIUM, "bold yellow"), (SEVERITY_LOW, "bold cyan")]:
    ...
    for a in top_alerts:
        ...
        desc  = a.description[:57] + "..." if len(a.description) > 60 else a.description
...
def do_single_scan(args, whitelist, blacklist, blacklist_patterns,
    ...
    print_section("STEP 4 - Unauthorized / Rogue Processes", Fore.CYAN)
    rogue = (
    ...
    print(f"  High-severity detections: {len(rogue)}")
    for a in rogue:
        print_alert_plain(a, use_color)
...
def _print_final_summary(engine: AlertEngine, use_color: bool):
    ...
    risk   = (
        ...
        "MODERATE" if medium >= 1 else "LOW"
    )

    c_h   = (Fore.RED    + Style.BRIGHT) if use_color else ""
    c_m   = (Fore.YELLOW + Style.BRIGHT) if use_color else ""
    c_l   = (Fore.CYAN   + Style.BRIGHT) if use_color else ""
    c_r   = ((Fore.RED if high > 0 else Fore.GREEN) + Style.BRIGHT) if use_color else ""
    reset = Style.RESET_ALL if use_color else ""

    print_section("SCAN COMPLETE - SUMMARY", Fore.GREEN if high == 0 else Fore.RED)
    print(f"  Risk Level   : {c_r}{risk}{reset}")
    print(f"  Total Alerts : {stats['total']}")
    print(f"  {c_h}HIGH   : {high}{reset}")
    print(f"  {c_m}MEDIUM : {medium}{reset}")
    print(f"  {c_l}LOW    : {low}{reset}")
